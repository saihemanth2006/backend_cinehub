...
class ProjectsPage extends StatefulWidget {
  ...
  final void Function(ProfileData) onAddRequest;
...
class _ProjectsPageState extends State<ProjectsPage> {
  ...
  List<ProfileData> get _filtered =>
      widget.allProfiles.asMap().entries.where((e) {
        final p = e.value;
        final isRequested = widget.collaborated.contains(e.key);
        
        // If they clicked "Requested", show ONLY requested profiles.
        if (_category == "Requested") return isRequested;

        // Otherwise, hide requested profiles from the other feeds.
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          ...
          final uName = auth.user?['fullName'] ?? auth.user?['username'] ?? 'User';
          final uRole = auth.user?['role'] ?? 'Collab Requested';
          
          widget.onAddRequest(ProfileData(
            id: auth.user?['_id'] ?? 'mock_new_request',
            name: uName,
            role: uRole,
            type: 'Collaborative',
            location: 'Remote',
            color1: const Color(0xffFF8C00),
            color2: const Color(0xffFF3D00),
            skills: ['Directing', 'Editing'],
            icon: Icons.movie_creation_rounded,
            followers: 0,
          ));
          _snack("Your collaboration request has been posted! 🎬");
        },

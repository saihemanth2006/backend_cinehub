...
class _MainScreenState extends State<MainScreen> with TickerProviderStateMixin {
  ...
  final Set<int> _collaborated = {};
  final Map<String, List<ChatMessage>> _conversations = {};
  List<ProfileData> _profiles = kAllProfiles;
  List<ProfileData> _activeRequests = [];
  ...
  Future<void> loadCollabRequests() async {
    try {
      final auth = AuthService();
      final reqs = await auth.fetchCollabRequests();
      ...
      final mapped = reqs.map((r) {
        final i = reqs.indexOf(r) % colors.length;
        final u = r['user'] is Map ? r['user'] : {};
        return ProfileData(
          id: r['_id']?.toString() ?? '',
          name: u['fullName'] ?? u['username'] ?? 'User',
          role: r['role'] ?? u['role'] ?? 'Collab Requested',
          type: r['type'] ?? 'Collaborative',
          location: r['location'] ?? 'Remote',
          color1: colors[i],
          color2: colors[(i + 1) % colors.length],
          skills: r['skills'] is List ? List<String>.from(r['skills']) : const [],
          icon: icons[i],
          followers: 0,
        );
      }).toList();
      ...
      if (!mounted) return;
      setState(() => _activeRequests = mapped);
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> pages = [
      ...
      // ── 1: Projects (Collaboration) ──────────────────────────────────────────
      ProjectsPage(
        allProfiles: _activeRequests,
        bookmarked: _bookmarked,
        collaborated: _collaborated,
        onBookmark: toggleBookmark,
        onCollaborate: addCollaboration,
        onAddRequest: (ProfileData req) async {
          final auth = AuthService();
          final savedReq = await auth.createCollabRequest(
            role: req.role,
            skills: req.skills,
          ...
          if (savedReq != null) {
            // Re-fetch to get populated user info correctly, or just prepend locally
            final u = savedReq['user'] is Map ? savedReq['user'] : auth.user;
            final newProfile = ProfileData(
              id: savedReq['_id']?.toString() ?? req.id,
              name: u?['fullName'] ?? u?['username'] ?? req.name,
              role: savedReq['role'] ?? req.role,
              type: savedReq['type'] ?? req.type,
              location: savedReq['location'] ?? req.location,
              color1: req.color1,
              color2: req.color2,
              skills: savedReq['skills'] is List ? List<String>.from(savedReq['skills']) : req.skills,
              icon: req.icon,
              followers: req.followers,
            );
            if (mounted) setState(() => _activeRequests.insert(0, newProfile));
          } else {
            // fallback if api fails
            if (mounted) setState(() => _activeRequests.insert(0, req));
          }
        },
      ),
